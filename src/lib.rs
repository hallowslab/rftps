pub mod config;
pub use config::{validate_directory, validate_username, Args};
pub mod utils;
pub use utils::{verify_home, resolve_local_ip, generate_random_string, validate_certificates};
pub mod auth;
pub mod logger;
...
impl FtpServer {
        ...
        if enable_ftps {
            ...
            if !provided_valid {
                ...
                #[cfg(feature = "include_pem_files")]
                {
                    ...
                    let mut t_cert = tempfile::NamedTempFile::new().map_err(|e| format!("Failed to create temp cert: {}", e))?;
                    let mut t_key = tempfile::NamedTempFile::new().map_err(|e| format!("Failed to create temp key: {}", e))?;
                    
                    t_cert.write_all(EMBEDDED_CERT).map_err(|e| format!("Failed to write temp cert: {}", e))?;
                    t_key.write_all(EMBEDDED_KEY).map_err(|e| format!("Failed to write temp key: {}", e))?;
    ...
    pub fn config(&self) -> (String, String, String) {
        (self.addr.to_string(), self.username.clone(), self.password.clone())
    }
    ...
    pub async fn run(self, mut stop_rx: oneshot::Receiver<()>) -> Result<(), String> {
        ...
        let server_builder = ServerBuilder::new(Box::new(move || {
            ...
            .notify_data(logger::DataLogger)
            .notify_presence(logger::ConnectionLogger);
        ...
        let server = server_builder.build().map_err(|e| format!("Error building FTP server: {}", e))?;
